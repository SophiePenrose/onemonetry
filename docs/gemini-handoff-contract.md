...
- `GET /api/gemini/handoff` (supports `status`, `has_response`, `has_retries`, `has_approvals`, `has_events`, `has_completed`, `before_accepted_at`, `after_accepted_at`, `sort`, `limit`, `offset`, `include_yamm_summary`, `include_status_counts`, `include_retry_counts`, `include_queue_metrics` query params)
