...
describe("enrichment-aware scoring", () => {
  ...
  it("uses fresh intent signals as bounded motion and timing boosts", () => {
    ...
    db.setSetting(`intent_signals_${enrichedCompany}`, {
      updated_at: isoDaysAgo(1),
      topics: ["checkout conversion", "payment orchestration"],
      motions: ["Merchant Acquiring", "Revolut Pay"],
      recency_days: 6,
      intent_signal_score: 0.82,
      confidence: "high",
      signals: [
        {
    ...
    assert.equal(enrichedScore.enrichment.intent.applied, true);
    assert.ok(Number(enrichedScore.enrichment.intent.motion_boosts["Merchant Acquiring"] || 0) > 0);
    assert.ok(Number(enrichedScore.enrichment.intent.motion_boosts["Revolut Pay"] || 0) > 0);
    assert.ok(Number(enrichedScore.enrichment.intent.urgency_boost || 0) > 0);
    assert.ok(Number(enrichedScore.enrichment.intent.propensity_boost || 0) > 0);
    assert.ok(Number(enrichedScore.all_motion_scores["Merchant Acquiring"]?.intent_boost || 0) <= 0.18);
    assert.ok(enrichedScore.layers.urgency.score > baselineScore.layers.urgency.score);
  });
