...
describe("API endpoints", () => {
  ...
  describe("Gemini handoff API stubs", () => {
    ...
    it("lists handoff requests with pagination and status filtering", async () => {
      ...
      const minApprovalCountList = await fetchJSON("/api/gemini/handoff?min_approval_count=1&limit=100&offset=0");
      assert.equal(minApprovalCountList.status, 200);
      assert.equal(minApprovalCountList.data.filters.min_approval_count, 1);
      ...
      assert.equal(maxApprovalCountList.data.items.length >= 1, true);
      assert.equal(maxApprovalCountList.data.items.every((entry) => Number(entry.approval_count || 0) <= 1), true);
      ...
      const hasLastRetryRequestedList = await fetchJSON("/api/gemini/handoff?has_last_retry_requested=true&limit=100&offset=0");
      assert.equal(hasLastRetryRequestedList.status, 200);
      assert.equal(hasLastRetryRequestedList.data.filters.has_last_retry_requested, true);
      assert.equal(hasLastRetryRequestedList.data.items.length >= 1, true);
      assert.equal(hasLastRetryRequestedList.data.items.every((entry) => Boolean(entry.last_retry_requested_at)), true);

      const noLastRetryRequestedList = await fetchJSON("/api/gemini/handoff?has_last_retry_requested=false&limit=100&offset=0");
      assert.equal(noLastRetryRequestedList.status, 200);
      assert.equal(noLastRetryRequestedList.data.filters.has_last_retry_requested, false);
    ...
    it("returns Gemini handoff operational summary", async () => {
      ...
      const summaryWithRecentRetryCounts = await fetchJSON("/api/gemini/handoff/summary?recent_hours=24&include_recent_retry_counts=true");
      assert.equal(summaryWithRecentRetryCounts.status, 200);
      assert.equal(typeof summaryWithRecentRetryCounts.data.recent_retry_counts, "object");
      assert.equal(typeof summaryWithRecentRetryCounts.data.recent_retry_counts.retry_limit, "number");
      ...
      const summaryWithRecentEventStageCounts = await fetchJSON("/api/gemini/handoff/summary?recent_hours=24&include_recent_event_stage_counts=true");
      assert.equal(summaryWithRecentEventStageCounts.status, 200);
      assert.equal(typeof summaryWithRecentEventStageCounts.data.recent_event_stage_counts, "object");
      ...
      const summaryWithRecentEventTypeShare = await fetchJSON("/api/gemini/handoff/summary?recent_hours=24&include_recent_event_type_share=true");
      assert.equal(summaryWithRecentEventTypeShare.status, 200);
      assert.equal(typeof summaryWithRecentEventTypeShare.data.recent_event_type_share, "object");
      assert.equal(typeof summaryWithRecentEventTypeShare.data.recent_event_type_share.total_events_recent, "number");
      assert.equal(Array.isArray(summaryWithRecentEventTypeShare.data.recent_event_type_share.top_event_types), true);
      assert.equal(
        summaryWithRecentEventTypeShare.data.recent_event_type_share.top_event_types.every((entry) => {
          return entry
      ...
      const summaryWithRecentCallbackAgingBands = await fetchJSON("/api/gemini/handoff/summary?recent_hours=24&include_recent_callback_aging_bands=true");
      assert.equal(summaryWithRecentCallbackAgingBands.status, 200);
      assert.equal(typeof summaryWithRecentCallbackAgingBands.data.recent_callback_aging_bands, "object");
      assert.equal(typeof summaryWithRecentCallbackAgingBands.data.recent_callback_aging_bands.total_open_requests, "number");
      assert.equal(typeof summaryWithRecentCallbackAgingBands.data.recent_callback_aging_bands.age_band_counts, "object");
      assert.equal(typeof summaryWithRecentCallbackAgingBands.data.recent_callback_aging_bands.age_band_counts.under_5m, "number");
      assert.equal(typeof summaryWithRecentCallbackAgingBands.data.recent_callback_aging_bands.age_band_counts.from_5m_to_30m, "number");
      assert.equal(typeof summaryWithRecentCallbackAgingBands.data.recent_callback_aging_bands.age_band_counts.from_30m_to_120m, "number");
      assert.equal(typeof summaryWithRecentCallbackAgingBands.data.recent_callback_aging_bands.age_band_counts.over_120m, "number");
      assert.equal(typeof summaryWithRecentCallbackAgingBands.data.recent_callback_aging_bands.age_band_counts_by_latest_callback_event, "object");

      const summaryWithRecentCallbackPayloadQualityCounts = await fetchJSON("/api/gemini/handoff/summary?recent_hours=24&include_recent_callback_payload_quality_counts=true");
      assert.equal(summaryWithRecentCallbackPayloadQualityCounts.status, 200);
      assert.equal(typeof summaryWithRecentCallbackPayloadQualityCounts.data.recent_callback_payload_quality_counts, "object");
      assert.equal(typeof summaryWithRecentCallbackPayloadQualityCounts.data.recent_callback_payload_quality_counts.completed_recent, "number");
      assert.equal(typeof summaryWithRecentCallbackPayloadQualityCounts.data.recent_callback_payload_quality_counts.parse_errors, "number");
      assert.equal(typeof summaryWithRecentCallbackPayloadQualityCounts.data.recent_callback_payload_quality_counts.missing_response_id, "number");
      assert.equal(typeof summaryWithRecentCallbackPayloadQualityCounts.data.recent_callback_payload_quality_counts.missing_completed_at, "number");
      assert.equal(typeof summaryWithRecentCallbackPayloadQualityCounts.data.recent_callback_payload_quality_counts.missing_status_field, "number");
      assert.equal(typeof summaryWithRecentCallbackPayloadQualityCounts.data.recent_callback_payload_quality_counts.status_ok, "number");
      assert.equal(typeof summaryWithRecentCallbackPayloadQualityCounts.data.recent_callback_payload_quality_counts.status_non_ok, "number");
      ...
      const summaryWithRecentYammRowGapCounts = await fetchJSON("/api/gemini/handoff/summary?recent_hours=24&include_recent_yamm_row_gap_counts=true");
      assert.equal(summaryWithRecentYammRowGapCounts.status, 200);
      assert.equal(typeof summaryWithRecentYammRowGapCounts.data.recent_yamm_row_gap_counts, "object");
      assert.equal(typeof summaryWithRecentYammRowGapCounts.data.recent_yamm_row_gap_counts.completed_recent, "number");
      assert.equal(typeof summaryWithRecentYammRowGapCounts.data.recent_yamm_row_gap_counts.parse_errors, "number");
      assert.equal(typeof summaryWithRecentYammRowGapCounts.data.recent_yamm_row_gap_counts.total_yamm_rows, "number");
      assert.equal(typeof summaryWithRecentYammRowGapCounts.data.recent_yamm_row_gap_counts.rows_missing_to, "number");
      assert.equal(typeof summaryWithRecentYammRowGapCounts.data.recent_yamm_row_gap_counts.rows_missing_subject, "number");
      assert.equal(typeof summaryWithRecentYammRowGapCounts.data.recent_yamm_row_gap_counts.rows_missing_body, "number");
      assert.equal(typeof summaryWithRecentYammRowGapCounts.data.recent_yamm_row_gap_counts.rows_missing_any_core_field, "number");
      assert.equal(typeof summaryWithRecentYammRowGapCounts.data.recent_yamm_row_gap_counts.rows_missing_multiple_core_fields, "number");
      assert.equal(typeof summaryWithRecentYammRowGapCounts.data.recent_yamm_row_gap_counts.approved_rows_missing_core_field, "number");
      assert.equal(typeof summaryWithRecentYammRowGapCounts.data.recent_yamm_row_gap_counts.approved_rows_almost_ready_missing_only_to, "number");
      ...
      const summaryWithRecentTransportErrorCodeCounts = await fetchJSON("/api/gemini/handoff/summary?recent_hours=24&include_recent_transport_error_code_counts=true");
      assert.equal(summaryWithRecentTransportErrorCodeCounts.status, 200);
      assert.equal(typeof summaryWithRecentTransportErrorCodeCounts.data.recent_transport_error_code_counts, "object");
      assert.equal(typeof summaryWithRecentTransportErrorCodeCounts.data.recent_transport_error_code_counts.total_failures, "number");
      assert.equal(typeof summaryWithRecentTransportErrorCodeCounts.data.recent_transport_error_code_counts.by_code, "object");
      assert.equal(
        Object.values(summaryWithRecentTransportErrorCodeCounts.data.recent_transport_error_code_counts.by_code)
          .every((value) => typeof value === "number"),
        true
      );

      const summaryWithRecentTransportOutcomeCounts = await fetchJSON("/api/gemini/handoff/summary?recent_hours=24&include_recent_transport_outcome_counts=true");
      assert.equal(summaryWithRecentTransportOutcomeCounts.status, 200);
      assert.equal(typeof summaryWithRecentTransportOutcomeCounts.data.recent_transport_outcome_counts, "object");
      assert.equal(typeof summaryWithRecentTransportOutcomeCounts.data.recent_transport_outcome_counts.total_retries, "number");
      ...
      const summaryWithQueueThroughputCounts = await fetchJSON("/api/gemini/handoff/summary?recent_hours=24&include_queue_throughput_counts=true");
      assert.equal(summaryWithQueueThroughputCounts.status, 200);
      assert.equal(typeof summaryWithQueueThroughputCounts.data.queue_throughput_counts, "object");
      assert.equal(typeof summaryWithQueueThroughputCounts.data.queue_throughput_counts.accepted, "number");
      assert.equal(typeof summaryWithQueueThroughputCounts.data.queue_throughput_counts.completed, "number");
      assert.equal(typeof summaryWithQueueThroughputCounts.data.queue_throughput_counts.retry_requested, "number");
    ...
    it("validates Gemini handoff summary query parameters", async () => {
      ...
      const invalidIncludeRecentCallbackAgingBands = await fetchJSON("/api/gemini/handoff/summary?include_recent_callback_aging_bands=maybe");
      assert.equal(invalidIncludeRecentCallbackAgingBands.status, 400);
      assert.equal(invalidIncludeRecentCallbackAgingBands.data.error, "invalid_include_recent_callback_aging_bands");
      ...
      const invalidIncludeRecentTransportErrorCodeCounts = await fetchJSON("/api/gemini/handoff/summary?include_recent_transport_error_code_counts=maybe");
      assert.equal(invalidIncludeRecentTransportErrorCodeCounts.status, 400);
      assert.equal(invalidIncludeRecentTransportErrorCodeCounts.data.error, "invalid_include_recent_transport_error_code_counts");
